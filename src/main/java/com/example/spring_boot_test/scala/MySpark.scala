...
object MySpark {
  ...
  def sparkStreamTest: Unit = {
    ...
    val fileRDD = ssc.textFileStream("D:\\projects\\spring_boot_test\\logFile2.txt").cache().map((_,1)).reduceByKey(_+_)
    fileRDD.print()
    ssc.start()
  ...
  /**
    ...
    * @return
    */
  def sparkTest: Array[Int] = {
    val listBuffer = ListBuffer[Int]()
    listBuffer.append(100)
    listBuffer.append(200)
    listBuffer.append(300)
    val list = listBuffer.toList
    val conf = new SparkConf().setAppName("javaUser")
    conf.setMaster("local")
    val sc = new SparkContext(conf)
    val rdd = sc.parallelize(list).map(_*10).collect()
    return rdd
  }
